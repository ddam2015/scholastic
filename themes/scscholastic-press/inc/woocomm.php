...
// 	End add multiple products
  ...
  //woocommerce wrappers
	...
	function scscholastic_wrapper_start() {
		echo '<section id="content" class="grid-x site-main woocomm-wrap" role="main">';
		echo '<div class="cell small-12">';
	}
	function scscholastic_wrapper_end() {
		echo '</div>';
		echo '</section>';
	}
	add_action('woocommerce_before_main_content', 'scscholastic_wrapper_start', 10);
	add_action('woocommerce_after_main_content', 'scscholastic_wrapper_end', 10);
	...
	function scscholastic_woocommerce_image_dimensions() {
		global $pagenow;
	...
	add_action( 'after_switch_theme', 'scscholastic_woocommerce_image_dimensions', 1 );

	//remove the breadcrumb (on product pages)
	function scscholastic_remove_wc_breadcrumbs() {
			remove_action( 'woocommerce_before_main_content', 'woocommerce_breadcrumb', 20, 0 );
	}
	add_action( 'init', 'scscholastic_remove_wc_breadcrumbs' );
  ...
  add_action( 'woocommerce_before_cart', 'woocommerce_checkout_login_form' );
	...
	function scscholastic_place_order_button_text() { return __( 'Complete Registration', 'woocommerce' ); }
	add_filter( 'woocommerce_order_button_text', 'scscholastic_place_order_button_text' );
  ...
  //change add to cart button text
	function scscholastic_woocommerce_product_add_to_cart_text(){
		return 'Register Now';
	}
	add_filter( 'woocommerce_product_add_to_cart_text' , 'scscholastic_woocommerce_product_add_to_cart_text' );
	add_filter( 'woocommerce_product_single_add_to_cart_text', 'scscholastic_woocommerce_product_add_to_cart_text' );
	add_filter( 'woocommerce_booking_single_add_to_cart_text', 'scscholastic_woocommerce_product_add_to_cart_text' );
...
function scscholastic_wc_scripts_styles() {
	if( is_admin() ) return;
...
add_action( 'wp_enqueue_scripts', 'scscholastic_wc_scripts_styles', 100);
...
function g365_add_event_id_reference() {
	wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/scscholastic-press/css/', 'https' ) . "style-admin.css?ver=2.4" );
...
function scscholastic_cart_process(){
  ...
  if( !empty(WC()->cart->get_cart()) ) {
    ...
    foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
      ...
      $extra_types = array( 'fee', 'gear', 'event', 'highsch', 'scholastics', 'tournaments' );
...
add_filter( 'woocommerce_before_cart' , 'scscholastic_cart_setup');
function scscholastic_cart_setup() {
  //load in global proc var
  global $cart_parse;
  //process cart items
  $cart_parse = scscholastic_cart_process();
  ...
  if(!empty($cart_parse[2]) ){
    ...
    add_filter( 'woocommerce_after_order_notes' , function( $fields ) use ( $order_data_field ) { return scscholastic_player_checkout_field( $field, $order_data_field ); });
  }
}
...
function scscholastic_player_checkout_field( $checkout, $extra_fields = null ) {
  ...
  echo '<input type="hidden" name="scscholastic_extra_data_required" value="' . implode(',',$added_field_names_required) . '" />';
  echo '<input type="hidden" name="scscholastic_extra_data" value="' . implode(',',$added_field_names) . '" />';
}

//make the extra data required based on the field names variable
add_action('woocommerce_checkout_process', 'scscholastic_player_checkout_field_process');
function scscholastic_player_checkout_field_process() {
  // Check if we have extra data
  if ( $_POST['scscholastic_extra_data_required'] ) {
    //parse and loop through the set to make sure we have all the fields filled out
    $fields = explode(',', $_POST['scscholastic_extra_data_required']);
...
add_action( 'woocommerce_checkout_update_order_meta', 'scscholastic_player_checkout_field_update_order_meta' );
function scscholastic_player_checkout_field_update_order_meta( $order_id ) {
  //join the extra_data list if we need to
  $scscholastic_extra_data = array();
  $scscholastic_extra_data_fields = array();
  if ( $_POST['scscholastic_extra_data'] ) {
    $scscholastic_extra_data[] = $_POST['scscholastic_extra_data'];
    $scscholastic_extra_data_fields[] = $_POST['scscholastic_extra_data'];
  }
  if ( $_POST['scscholastic_extra_data_required'] ) {
    $scscholastic_extra_data[] = $_POST['scscholastic_extra_data_required'];
    $scscholastic_extra_data_fields[] = $_POST['scscholastic_extra_data_required'];
  }
  //save the field reference so we know what to pull later
  if ( !empty($scscholastic_extra_data) ) update_post_meta( $order_id, 'scscholastic_extra_data', sanitize_text_field( implode(',', $scscholastic_extra_data) ) );
  // Check if we have extra data
  if ( !empty($scscholastic_extra_data_fields) ) {
    //parse extra field names, then loop and save
    $fields = explode(',', implode(',', $scscholastic_extra_data_fields));
    foreach( $fields as $dex => $key ) {
...
add_action( 'woocommerce_admin_order_data_after_billing_address', 'scscholastic_player_checkout_field_display_admin_order_meta', 10, 1 );
function scscholastic_player_checkout_field_display_admin_order_meta($order){
  //parse the field names, if we have them, then echo them
  $fields = get_post_meta( $order->id, 'scscholastic_extra_data', true );
  if( !empty($fields) ) {
...
function scscholastic_edit_memberships_actions( $actions ) {
...
add_filter( 'wc_memberships_members_area_my-memberships_actions', 'scscholastic_edit_memberships_actions' );
add_filter( 'wc_memberships_members_area_my-membership-details_actions', 'scscholastic_edit_memberships_actions' );
    ...
    foreach ( $product_ids as $product_id ) {
      ...
      if( !empty($series_events[ $product_id ][ 'data_pull' ]) ) {
        ...
        if( empty($series_events[ $product_id ][ 'data_pull' ]->dates) ) {
          ...
          //add to the series order so we can be organized
          $series_order[ $product_id ] = date( 'Y-m-d', strtotime(scscholastic_build_dates($series_events[ $product_id ][ 'data_pull' ]->dates, 4)));
          ...
          if( $payment_options ) {
            ...
            $series_events[ $product_id ][ 'label' ][] = scscholastic_build_dates($series_events[ $product_id ][ 'data_pull' ]->dates, 1, true);
...
add_filter( 'woocommerce_checkout_fields' , 'scscholastic_modify_checkout_fields' );
// Our hooked in function - $fields is passed via the filter!
function scscholastic_modify_checkout_fields( $fields ) {
   unset($fields['order']['order_comments']);
   return $fields;
}

//validate the checkout form before finishing the process of writting data to scscholastic about the purchase
add_action('woocommerce_after_checkout_validation', 'process_scscholastic_data');
function process_scscholastic_data($form_data) {
...
add_action( 'woocommerce_checkout_update_order_meta', 'scscholastic_update_meta_form_fields' );
function scscholastic_update_meta_form_fields( $order_id ) {
  if ( ! empty( $_POST['order_data'] ) ) update_post_meta( $order_id, '_order_data', $_POST['order_data'] );
}

//display and custom order data for admin
add_action( 'woocommerce_admin_order_data_after_shipping_address', 'scscholastic_checkout_field_display_admin_order_meta', 10, 1 );
function scscholastic_checkout_field_display_admin_order_meta($order){
...
// function OLD_scscholastic_checkout_field_display_admin_order_meta($order){
//   //for setting form restrictions/helpers
//   $scscholastic_cart_items = array();
//   //return string
//   $to_write = '';
...
//       // update functionality
//       //
//   wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/scscholastic-press/css/', 'https' ) . "style-admin.css?ver=2.345" );
// 	wp_enqueue_script( 'admin-jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js', array(), '2.2.4', true );
//   wp_enqueue_script( 'js-g365-all', (( strpos(site_url(), 'dev.') === false ) ? 'https://grassroots365.com/data-processor.js' : 'https://dev.grassroots365.com/data-processor.js'), array('admin-jquery'), '695450', true );
...
add_action( 'show_user_profile', 'scscholastic_user_fields_display' );
add_action( 'edit_user_profile', 'scscholastic_user_fields_display' );

function scscholastic_user_fields_display( $user ) {
	wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/scscholastic-press/css/', 'https' ) . "style-admin.css?ver=2.345" );
	wp_enqueue_script( 'admin-jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js', array(), '2.2.4', true );
  wp_enqueue_script( 'js-g365-all', (( strpos(site_url(), 'dev.') === false ) ? 'https://grassroots365.com/data-processor.js' : 'https://dev.grassroots365.com/data-processor.js'), array('admin-jquery'), '695450', true );
  ?>
	<h3><?php esc_html_e( 'Additional User Data', 'scscholastic' ); ?></h3>
  <div class="options_group g365_manage_wrapper">
...
add_action( 'user_profile_update_errors', 'scscholastic_user_field_error', 10, 3 );
function scscholastic_user_field_error( $errors, $update, $user ) {
	if ( ! $update ) {
		return;
	}
}

add_action( 'personal_options_update', 'scscholastic_user_fields_update' );
add_action( 'edit_user_profile_update', 'scscholastic_user_fields_update' );

function scscholastic_user_fields_update( $user_id ) {
