...
function pl_waiver_columns( $columns ) {
  ...
  $columns['scscholastic_user_name'] = 'User Name';
  $columns['scscholastic_user_email'] = 'User Email';
  return $columns;
}
add_filter( 'manage_nf_sub_posts_columns', 'pl_waiver_columns' );

function pl_waiver_column( $column, $sub_id ) {
  if( $column === 'scscholastic_user_name' || $column === 'scscholastic_user_email' ) {
    //get submission data
    $sub = Ninja_Forms()->form()->get_sub( $sub_id );
    //pull user info
    $user = $sub->get_user();
    //add it in to the column data
    if( $column === 'scscholastic_user_name') echo $user->data->user_nicename;
    if( $column === 'scscholastic_user_email') echo $user->data->user_email;
  }
}
...
function add_apple_touch_icons(){
	...
	echo '<link rel="icon" type="image/png" href="' . get_template_directory_uri() . '/assets/tiny-logos/Scholastic-Series-400x300.png" />' . "\n";
...
if ( ! function_exists( 'theme_setup' ) ) :
    ...
    add_theme_support( 'align-wide' );
		...
		load_theme_textdomain( 'scscholastic-press' );
...
if ( ! function_exists( 'scscholastic_main_nav' ) ) {
	function scscholastic_main_nav() {
    ...
    wp_nav_menu( array(
			...
			'fallback_cb'    => false,
			'walker'         => new scscholastic_Top_Bar_Walker()
		));
	}
}
// Main Navigation Mega
if ( ! function_exists( 'scscholastic_mega_nav' ) ) {
	function scscholastic_mega_nav() {
		wp_nav_menu( array(
			'theme_location' => 'main_nav',
			'container'      => false,
			...
			'fallback_cb'    => false,
			'walker'         => new scscholastic_Mega_Walker()
		));
	}
}
// Main Navigation side Slide
if ( ! function_exists( 'scscholastic_side_slide_nav' ) ) {
	function scscholastic_side_slide_nav() {
		wp_nav_menu( array(
			'theme_location' => 'main_nav',
			'container'      => false,
			...
			'fallback_cb'    => false,
			'walker'         => new scscholastic_Side_Slide_Walker()
		));
	}
}
// Title Nav
if ( ! function_exists( 'scscholastic_title_nav' ) ) {
	function scscholastic_title_nav() {
		wp_nav_menu( array(
			'theme_location' => 'title_nav',
			'container'      => false,
...
// Footer Nav
if ( ! function_exists( 'scscholastic_footer_nav' ) ) {
	function scscholastic_footer_nav() {
		wp_nav_menu( array(
...
if ( ! function_exists( 'scscholastic_excerpt' ) ) :
	/**
	 * Displays the optional excerpt.
	 * Wraps the excerpt in a div element.
	 * Create your own scscholastic_excerpt() function to override in a child theme.
	 * @param string $class Optional. Class string of the div element. Defaults to 'entry-summary'.
	 */
	function scscholastic_excerpt( $class = 'entry-summary' ) {
...
if ( ! function_exists( 'scscholastic_excerpt_more' ) && ! is_admin() ) :
/**
 * Replaces "[...]" (appended to automatically generated excerpts) with ... and a 'Continue reading' link.
 * Create your own scscholastic_excerpt_more() function to override in a child theme.
 * @return string 'Continue reading' link prepended with an ellipsis.
 */
function scscholastic_excerpt_more() {
	$link = sprintf( '<a href="%1$s" class="more-link">%2$s</a>',
		esc_url( get_permalink( get_the_ID() ) ),
		/* translators: %s: Name of current post */
		sprintf( __( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'scscholastic-press' ), get_the_title( get_the_ID() ) )
	);
	return ' &hellip; ' . $link;
}
add_filter( 'excerpt_more', 'scscholastic_excerpt_more' );
endif;

if ( ! function_exists( 'scscholastic_entry_meta' ) ) :
/**
 * Prints HTML with meta information for the categories, tags.
 * Create your own scscholastic_entry_meta() function to override in a child theme.
 */
function scscholastic_entry_meta() {
	if ( 'posts' === get_post_type() ) {
		$author_avatar_size = apply_filters( 'scscholastic_author_avatar_size', 49 );
		printf( '<span class="byline"><span class="author vcard">%1$s <span class="screen-reader-text">%2$s </span> <a class="url fn n" href="%3$s">%4$s</a></span></span>',
			get_avatar( get_the_author_meta( 'user_email' ), $author_avatar_size ),
			_x( 'Author', 'Used before post author name.', 'scscholastic-press' ),
	...
	if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
		scscholastic_entry_date();
	}

	$format = get_post_format();
	if ( current_theme_supports( 'post-formats', $format ) ) {
		printf( '<span class="entry-format">%1$s<a href="%2$s">%3$s</a></span>',
			sprintf( '<span class="screen-reader-text">%s </span>', _x( 'Format', 'Used before post format.', 'scscholastic-press' ) ),
	...
	if ( 'posts' === get_post_type() ) {
// 		scscholastic_entry_taxonomies();
	}

	if ( ! is_singular() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( sprintf( __( 'Leave a comment<span class="screen-reader-text"> on %s</span>', 'scscholastic-press' ), get_the_title() ) );
		echo '</span>';
	}
}
endif;

if ( ! function_exists( 'scscholastic_entry_date' ) ) :
/**
 * Prints HTML with date information for current post.
 * Create your own scscholastic_entry_date() function to override in a child theme.
 */
function scscholastic_entry_date() {
...
// 		$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time> | <time class="updated" datetime="%3$s">%4$s</time>';
	...
	printf( '<span class="posted-on"><span class="screen-reader-text">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span>',
		_x( 'Posted on', 'Used before publish date.', 'scscholastic-press' ),
...
function scscholastic_admin_menu() {
	add_menu_page( 'OGP Data Admin', 'Data', 'manage_options', 'scscholastic_data', 'scscholastic_admin', 'dashicons-chart-line', 26  );
// 	add_submenu_page( 'scscholastic_data', 'scscholastic Data Dashboard', 'Dashboard', 'manage_options', 'scscholastic_data', 'scscholastic_admin' );
}
add_action( 'admin_menu', 'scscholastic_admin_menu' );

//build admin page
function scscholastic_admin(){
	wp_enqueue_style( 'scscholastic_admin_styles', get_stylesheet_directory_uri() . '/css/scscholastic-admin-style.css' );
	$g_action = filter_input( INPUT_GET, 'g_action', FILTER_SANITIZE_URL );
	$g_action = ( empty($g_action) ) ? 'settings' : $g_action;
	$scscholastic_sections = array(
		'settings' => array(
			'title'		=> 'Manage Site Settings',
			'tab_name'	=> 'Settings',
			'url'			=> '?page=scscholastic_data&g_action=settings'
		)
	);
	echo '<div class="scscholastic_data_manager_wrapper">';
	$scscholastic_admin_url = '';
	$nav = '';
	foreach( $scscholastic_sections as $section => $section_data ) {
		if( $section == $g_action ) {
			echo '<h1>' . $section_data['title'] . '</h1>';
			$scscholastic_admin_url = $section_data['url'];
	...
	echo '<nav class="nav-tab-wrapper">' . $nav . '</nav><div class="scscholastic_data_manager_content_wrapper">';
	switch( $g_action ) {
		case 'export_data':
			scscholastic_export();
			break;
		case 'record_data':
			scscholastic_record_stats(); 
			break;
		case 'settings':
			$scscholastic_admin_settings = array(
				'display' => array(
			...
			function scscholastic_update_admin_site_settings( $old, $new ) {
				foreach( $new as $item => $item_data ) {
					foreach( $item_data as $item_name => $item_value ) {
						$old[$item]['items'][$item_name]['value'] = scscholastic_process_data_point( $old[$item]['items'][$item_name]['type'], $item_value);
            if( $old[$item]['items'][$item_name]['value'] === null || $old[$item]['items'][$item_name]['value'] === 'null' ) $old[$item]['items'][$item_name]['value'] = '';
					}
				}
				return $old;
			}
			foreach( $scscholastic_admin_settings as $setting_set => &$setting_data ) {
				if( $_POST['g_process'] == 'process' ) {
					$scscholastic_db_option = $_POST['scscholastic_admin_form_data'][$setting_set];
					$update = update_option( 'scscholastic_' . $setting_set, $scscholastic_db_option );
					$setting_data['section_records'] = scscholastic_update_admin_site_settings( $setting_data['section_records'], $scscholastic_db_option );
					if( $update === true ) echo '<p class="success">Updated settings data!</p>';
				} else {
					$scscholastic_db_option = get_option( 'scscholastic_' . $setting_set );
					if( $scscholastic_db_option !== false && !empty($scscholastic_db_option) ) $setting_data['section_records'] = scscholastic_update_admin_site_settings( $setting_data['section_records'], $scscholastic_db_option );
        }
			} ?>
			<h3>General Site Settings</h3>
			<form id="scscholastic_form" method="post" class="scscholastic_form" action="<?php echo $scscholastic_admin_url; ?>">
				<input type="hidden" name="g_process" value="process" />
			<?php foreach( $scscholastic_admin_settings as $setting_set_re => $setting_data_re ) : ?>
				...
				<table class="scscholastic_form_section form-table">
					<tbody>
					<?php foreach( $record_data['items'] as $element => $element_data ) {
						$element_data['tag'] = 'scscholastic_admin_form_data[' . $setting_set_re . '][' . $record . '][' . $element . ']';
						$element_data['description'] = ( empty($element_data['description']) ) ? '' : '<small>' . $element_data['description'] . '</small>';
						echo scscholastic_template_construction( $element_data );
					} ?>
					</tbody>
				</table>
				<?php endforeach;
...
function g365_data_receiver(){
  $grassroots_keys = get_option( 'scscholastic_g365_connector' );
...
function send_html_email($email, $subject, $message) {
  $headers = ( strpos( site_url(), 'dev' ) !== false ) ? array('From: scscholastic Dev Customer Service <no-reply@dev.elitebasketballcircuit.com>') : array('From: scscholastic Customer Service <no-reply@elitebasketballcircuit.com>');
...
// 	$pageID = ( $pageID === null ) ? $post->ID : $pageID;
	...
	$scscholastic_site_ads = get_option( 'scscholastic_display' );
	//if the page doesn't have an ad or the site global is empty, don't add to array
	$ad_info['go'] = ( empty($scscholastic_page_ad['link']) || empty($scscholastic_page_ad['img']) || empty($scscholastic_site_ads) ) ? false : true;
	...
	if ( $ad_info['go'] ) {
		...
		$scscholastic_site_ads['site_1']['element_type'] = 'rotator';
		$scscholastic_site_ads['site_2']['element_type'] = 'rotator';
		//build rotator element for page ad
		$ad_info['ad_content'] .= scscholastic_template_construction( $scscholastic_page_ad );
		//if we have a global ad, build and add it
		if( !empty($scscholastic_site_ads['site_1']['img']) ) $ad_info['ad_content'] .= scscholastic_template_construction( $scscholastic_site_ads['site_1'] );
		//if we have a secondary global ad, build if and add it along with another primary ad to help the flow
		if( !empty($scscholastic_site_ads['site_2']['img']) ) {
			$ad_info['ad_content'] .= scscholastic_template_construction( $scscholastic_page_ad );
			$ad_info['ad_content'] .= scscholastic_template_construction( $scscholastic_site_ads['site_2'] );
		}
	}
	//on the front page, and if we have a global splash setting
	if( is_front_page() && !empty($scscholastic_site_ads['site_3']) ) {
		//controls for the splash
		$scscholastic_splash = array(
			'title'				=> ( empty($scscholastic_site_ads['site_3']['title']) ) ? '' : $scscholastic_site_ads['site_3']['title'],
			'link'				=> ( empty($scscholastic_site_ads['site_3']['link']) ) ? '' : $scscholastic_site_ads['site_3']['link'],
			'img'					=> ( empty($scscholastic_site_ads['site_3']['img']) ) ? '' : $scscholastic_site_ads['site_3']['img'],
			'img_mobile'	=> ( empty($scscholastic_site_ads['site_3']['img_mobile']) ) ? '' : $scscholastic_site_ads['site_3']['img_mobile'],
			'description'	=> ( empty($scscholastic_site_ads['site_3']['description']) ) ? '' : $scscholastic_site_ads['site_3']['description']
		);
		//if we have minimum info process the splash
		if( !empty($scscholastic_splash['title']) && !empty($scscholastic_splash['link']) && !empty($scscholastic_splash['img']) ) {
			//build the splash html
			$ad_info['splash'] = '<div class="reveal text-center" id="scscholastic_home_reveal" aria-labelledby="' . $scscholastic_splash['title'] . '" data-reveal><div class="relative"><h1 id="reveal-title" class="show-for-sr">' . $scscholastic_splash['title'] . '</h1>';
			//if there is a derscription, add it
			if( !empty($scscholastic_splash['description']) ) $ad_info['splash'] .= '<p class="reveal-description">'. $scscholastic_splash['description'] . '</p>';
			//if there is a mobile image, add it
			if( !empty($scscholastic_splash['img_mobile']) ) $ad_info['splash'] .= '<p class="text-center show-for-small-only"><a href="' . $scscholastic_splash['link'] . '"><img src="'. $scscholastic_splash['img_mobile'] . '" alt="'. $scscholastic_splash['title'] . '" title="'. $scscholastic_splash['title'] . '" /></a></p>';
			//if there is a mobile image, add supporting class to the large image
			$mobile_img_class = ( empty($scscholastic_splash['img_mobile']) ) ? '' : ' show-for-medium';
			//add main splash image
			$ad_info['splash'] .= '<p class="text-center' . $mobile_img_class . '"><a href="' . $scscholastic_splash['link'] . '"><img src="'. $scscholastic_splash['img'] . '" alt="'. $scscholastic_splash['title'] . '" title="'. $scscholastic_splash['title'] . '" /></a></p>';
...
function scscholastic_template_construction( $ele_data ) {
	$scscholastic_admin_form_elements = array(
		'input' 	=> array(
			'template' => '<tr>
					<th>
	...
	$ele = $scscholastic_admin_form_elements[$ele_data['element_type']]['template'];
	foreach( $scscholastic_admin_form_elements[$ele_data['element_type']]['vars'] as $dex => $var ) {
    if( $var === 'options' ) {
      $all_options = '';
      $option_ele = $scscholastic_admin_form_elements[$ele_data['element_type']]['template_option'];
...
function scscholastic_process_data_point( $name, $point ) {
...
function scscholastic_build_dates($dates, $type = 1, $abbv = false, $add_reg = false) {
    ...
    switch( $type ){
      ...
      case 'date':
        ...
        if( empty($event_object->dates) ) {
          ...
          $date_reg = scscholastic_build_dates($event_object->dates, 2, false, $reg_days);
          if( $reg_days === false ) {
      ...
      case 'full_address':
        if( empty($event_object->short_locations) ) {
          ...
          $reg_days = $atts['reg_days'];
          $date_reg = scscholastic_build_dates($event_object->short_locations, 2, false, $reg_days);
          if( $reg_days === false ) {
            $data_compile .= $date_reg;
          } else {
            $data_compile .= $date_reg[0];
