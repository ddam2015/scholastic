...
if ($current_month > 8) {
    $season_year = $current_year . '-' . (date('Y', strtotime('+1 year', strtotime($current_year))));
} else {
    $season_year = (date('Y', strtotime('-1 year', strtotime($current_year)))) . '-' . $current_year;
}
?>
<div class="grid-container">
  ...
  <?php 
    switch( $pg_type ){
      case '':
      case 'all-scs-events':
      case 'event': scs_dir_render('stat-leaderboard','by-event', '', $arg = null); break;
    }
  ?>
